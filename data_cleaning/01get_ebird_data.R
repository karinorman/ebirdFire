...
# need a dataframe that lists the layers available for each species
species_year <- bind_rows(
  tibble(year = 2021, species_code = list.files(here::here("raw_data", "2021"))),
  tibble(year = 2022, species_code = list.files(here::here("raw_data", "2022")))) %>%
...
species_layers <- purrr::pmap_dfr(species_year, function(year, species_code, path){

  #browser()

  bird_rast <- terra::rast(path)

  df <- tibble(layers = names(bird_rast), yes = 1) %>%
    tidyr::pivot_wider(names_from = layers, values_from = yes) %>%
    mutate(year = year, species_code = species_code, path = path)
})

usethis::use_data(species_layers)
