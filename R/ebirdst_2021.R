...
#'                         dry_run = TRUE)
#' }
ebirdst_download_status_2021 <- function(species,
  ...
  files <- get_download_file_list_2021(species_code = species, path = path)
  # path to data package
  run_path <- file.path(path, 2021, species)
...
#' }
ebirdst_download_trends_2021 <- function(species,
                                    path = ebirdst_data_dir(),
                                    force = FALSE,
                                    show_progress = TRUE) {
  ...
  for (s in species_code) {
    # complete list of all available files for this species
    files <- get_download_file_list_2021(species_code = s, path = path)
    # path to data package
    run_path <- file.path(path, 2021, s)
...
#' }
get_species_path_2021 <- function(species, path = ebirdst_data_dir(),
...
get_download_file_list_2021 <- function(species_code, path) {
